...
**Facade** (`DataFacade.js`, browser y Node.js). Fue el patrón que más simplificó el código de los módulos de interfaz: `App.Indicadores`, `App.Datos` y los demás llaman a métodos de alto nivel (`facade.getVentas()`, `facade.getInventario()`) sin conocer puertos, rutas ni manejo de errores HTTP. Esto también facilitó los tests: hay una única interfaz pública con ocho métodos (`getUserData`, `getDashboardData`, `getVentas`, `getIndicadores`, `getReportes`, `getInventario`, `getEmpleados`, `getEventos`), y cubrir esos ocho métodos con mocks del fetcher fue más simple que testear ocho llamadas directas con URLs diferentes.
...
Cobertura global medida por Jest: **86.71% de sentencias**, **81.96% de funciones** y **88.99% de líneas** — las tres por sobre el umbral mínimo real de 70% (`coverageThreshold.global` en `package.json`). La cobertura de **branches (69.86%)** queda por debajo de ese mismo umbral, por lo que el gate de `npm test` está fallando en esa métrica; es una deuda de testing pendiente, no un dato al que quepa aplicarle un ✅ sin matices.
